...
router.post(
  "/watch-history",
  tokenMiddleware.auth,
  userController.addToWatchHistory
)

router.delete(
  "/watch-history",
  tokenMiddleware.auth,
  userController.clearWatchHistory
)

router.put(
  "/mark-first-login-complete",
  tokenMiddleware.auth,
  userController.markFirstLoginComplete
)

router.get(
  "/check-new-episodes",
  tokenMiddleware.auth,
  userController.checkForNewEpisodes
)

router.put(
  "/mark-episode-notification-seen",
  tokenMiddleware.auth,
  body("mediaId")
    .exists().withMessage("mediaId is required")
    .isLength({ min: 1 }).withMessage("mediaId can not be empty"),
  requestHandler.validate,
  userController.markEpisodeNotificationSeen
)
